...
(() => {
  ...
  function bind(){
    ...
    $("#sendMagicLinkBtn").onclick=async()=>{
      ...
      const {error}=await sb.auth.signInWithOtp({email,options:{emailRedirectTo:location.href.split("#")[0]}}); error?toast(error.message,true):toast("Secure sign-in link sent. Check your email.");
    };
